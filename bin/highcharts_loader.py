...
class Options:
    ...
    def __init__(self, *, from_file=None, from_dict=None):
        ...
        if from_file and from_dict:
            raise ToManyParams(
                'You should pass only one param: "from_file" or "from_dict".'
            )
...
class ChartLoader:
    ...
    def __init__(self, options: Options, image_type="image/png"):
        ...
        response = requests.post(
            self.url, data={"type": image_type, "options": json.dumps(options.data)}
        )
    ...
    def get_data_image(self):
        return "data:image/{0};charset=utf-8;base64,{1}".format(
            self.image_type, self._decoded_chart()
        )
