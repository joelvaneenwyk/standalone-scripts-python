...
def main():
    if user == "default":
        print(
            "Please define the twitter user's screenname to download content from in source code."
        )
        return
    ...
    curr_id = tweets[-1].id
    for i in range(19):
        tweets = tweets + api.GetUserTimeline(
            screen_name=user, count=200, max_id=curr_id
        )
        curr_id = tweets[-1].id
